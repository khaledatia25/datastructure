...
template <class T>
void Vector<T>::insert(int idx, T value) {
  assert(0 <= idx && idx < size);
  if (size == capacity) expand_capacity();
...
template <class T>
void Vector<T>::rotate(int times) {
  times %= size;
  if (times == 0) return;
  int* arr2 = new int[times]{};
  for (int j = 0, i = size - times; i < size; i++, j++) arr2[j] = arr[i];
  for (int i = 0; i < size - times; i++) arr[i + times] = arr[i];
  for (int i = 0; i < times; i++) arr[i] = arr2[i];
  delete[] arr2;
}
